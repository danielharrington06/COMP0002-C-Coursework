...
- Once reaching the edge of the arena or an obstacle, it spirals around, keeping the tiles it knows (visited tiles and obstacles) to its left and if the tile to its left is unknown, it attempts to visit it
...
This algorithm guarantees that the robot visits every tile in the arena (so long as it physically can) and does so in an efficient way such that it can minimise the number of tiles it repeats for abstract and complicated obstacle placements. It also looks pretty cool and very clean!
...
In the source code, to access a specific coordinate `(x, y)`, `arenaGrid[y][x]` is used as this implementation stores rows together so the coordinates must be flipped to avoid segmentation faults.
